    ...
    <div class="main row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h3 class="mb-0">Potencia tus decisiones con la precisión de nuestros bots.</h3>
            </div>
                    ...
                    <div class="search-bar" style="background-color:#444746;">
                        <form id="myForm">
                            @php
                                $id = request('id');
                            @endphp
                            <input hidden name="id_bots" id="id_bots" value="<?= $id ?>" required>
                            <div class="row g-3">
                                <div class="col-lg-8 col-md-12 col-sm-12">
                                    <div class="input-box">
                                        <label for="nombre" class="darkblue-text-bold">Bots</label>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(event) {
                ...
                setTimeout(function() {
                    ...
                    fetch('/api/agregar_bots_usuario', {
                        ...
                        .then(data => {
                            ...
                            if (data.status === 201) {
                                ...
                                const redirectUrl =
                                    `{{ route('form_lista_bots') }}?Message=${encodeURIComponent(Message)}`;
                                form.reset();
                                button.disabled = false;
            ...
            const id = document.getElementById('id_bots').value;


            // Realizar la solicitud Fetch a tu API en api.php
            fetch(`/api/seleccion_bots_compra/${id}`, {
                    method: 'GET',
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
                        'Accept': 'application/json',
                ...
                .then(data => {
                    ...
                    document.getElementById('nombre').value = data.data[0].botNombre;
                    document.getElementById('valor').value = data.data[0].botValor + ' USD';
                    document.getElementById('ficha').value = data.data[0].botValor;
                    document.getElementById('descripcion').innerText = data.data[0].botDescripcion;
