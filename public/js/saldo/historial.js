...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fillTableWithData(data) {
        ...
        data.data.forEach((usu_sal, index) => {
            const row = document.createElement('tr');
            row.innerHTML = `
            <td>${index + 1}</td>
            <td>${usu_sal.his_sal_fecha}</td>
            <td>${usu_sal.his_sal_observacion}</td>
            <td>${usu_sal.usuLogin}</td>
            <td class="${usu_sal.his_sal_observacion === 'Transferencia Realizada' ? 'text-danger' : ''}">${usu_sal.his_sal_monto}</td>`;
    ...
    fetch('/api/his_sal/' + idUsuario,
        ...
        .then(data => {
            if (data.data.length > 0) {
                ...
                noDataMessage.innerHTML = '<td colspan="5"><center>No hay datos para mostrar.</center></td>';
    ...
    filterField.addEventListener('input', function () {
        ...
        if (searchValue === '' || searchValue.trim() === '') {
            ...
            if (originalData && originalData.data) { // Asegura que originalData y originalData.data estén definidos
                ...
                const filteredData = originalData.data.filter(usu_sal =>
                    usu_sal.his_sal_fecha.toLowerCase().includes(searchValue)
                );
