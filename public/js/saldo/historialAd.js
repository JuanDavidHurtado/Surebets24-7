...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fillTableWithData(data) {
        ...
        data.data.forEach((usu_sal, index) => {
            ...
            row.innerHTML = `
            <td>${index + 1}</td>
            <td>${usu_sal.fecha_transferencia}</td>
            <td>${usu_sal.emisor_login}</td>
            <td>${usu_sal.receptor_login}</td>
            <td>${usu_sal.descripcion}</td>
            <td>${usu_sal.cantidad_fichas}</td>
           `;
    ...
    fetch('/api/his_adm',
        ...
        .then(data => {
            if (data.data.length > 0) {
                ...
                noDataMessage.innerHTML = '<td colspan="6"><center>No hay datos para mostrar.</center></td>';
        ...
        if (searchValue === '' || searchValue.trim() === '') {
            ...
            if (originalData && originalData.data) { // Asegura que originalData y originalData.data estén definidos
                ...
                const filteredData = originalData.data.filter(usu_sal =>
                    usu_sal.fecha_transferencia.toLowerCase().includes(searchValue)
                );
