...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fillTableWithData(data) {
        ...
        data.data.forEach((comision, index) => {
            ...
            row.innerHTML = `
            <td>${index + 1}</td>
            <td>${comision.bot_fecha}</td>
            <td>${comision.usuNombre + ' ' + comision.usuApellido}</td>
            <td>${comision.botNombre + ' ' + comision.botValor} USDT</td>
            <td>${comision.comValor} USDT</td>
            <td>${comision.botPorcentaje} %</td>
            <td><span class="badge ${comision.usu_bot_estado === 'FINALIZADO' ? 'bg-success' : 'bg-primary'}">${comision.usu_bot_estado}</span></td>
                  `;
    ...
    fetch('/api/lis_com_bot/' + idUsuario,
        {
            method: 'GET',
            headers: {
