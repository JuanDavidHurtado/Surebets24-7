...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fillTableWithData(data) {
        ...
        data.data.forEach((usu_inv, index) => {
            ...
            row.innerHTML = `
                <td>${index + 1}</td>
                <td> ${"Solicitud: " + usu_inv.usu_pag_fecha + "<br>Fec. Inicial: " + usu_inv.inv_fecha_inicio + "<br>Fec. Final: <span class='bg-primary'>" + usu_inv.inv_fecha_final}</span></td>
                <td>${usu_inv.usuDocumento + "<br>" + usu_inv.usuNombre + " " + usu_inv.usuApellido}</td>
                <td>${'Banco: ' + usu_inv.metBanco + '<br>Tipo: ' + usu_inv.metTipo + '<br>#: ' + usu_inv.metNumero}</td>
                <td>${usu_inv.usu_pag_monto + " USD"}</td>
                <td>${usu_inv.invNombre + "<br>" + usu_inv.invValor + "USD<br>" + usu_inv.inv_dias} Dias</td>
                <td>${usu_inv.total_pago} USD</td>
                <td>${usu_inv.usu_inv_estado}</td>
                <td>
                <button class="badge bg-primary" onclick="actualizar('REALIZADO', ${usu_inv.id_usu_pag})">Aceptar</button>
                <button class="badge bg-danger" onclick="actualizar('ANULADO', ${usu_inv.id_usu_pag})">Anular</button>
                </td>
    ...
    fetch('/api/pendiente_adelanto',
        ...
        .then(data => {
            if (data.data.length > 0) {
                ...
                noDataMessage.innerHTML = '<td colspan="9"><center>No hay adelantos de inversion pendientes en el sistema en este momento.</center></td>';
