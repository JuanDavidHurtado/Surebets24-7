...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fillTableWithData(data) {
        ...
        data.data.forEach((usuario, index) => {
            ...
            row.innerHTML = `
                ...
                <td>
                ${usuario.usuGiro === 'SI'
                    ? `<button class="badge bg-danger" onclick="actualizar_estado_giro('NO', ${usuario.idUsuario})">NO</button>`
                    : `<button class="badge bg-primary" onclick="actualizar_estado_giro('SI', ${usuario.idUsuario})">SI</button>`}
    ...
    fetch('/api/lis_usu', {
        ...
        .then(data => {
            if (data.data.length > 0) {
                ...
                noDataMessage.innerHTML = '<td colspan="9"><center>No hay usuarios registrados en el sistema en este momento.</center></td>';
    ...
    filterField.addEventListener('input', function () {
        ...
        if (searchValue === '' || searchValue.trim() === '') {
            ...
            if (originalData && originalData.data) { // Asegura que originalData y originalData.data estén definidos
                ...
                const filteredData = originalData.data.filter(usuario =>
    ...
    // Función para actualizar el método al hacer clic en los botones
    window.actualizar_estado_giro = function (nuevoEstado, id) {
